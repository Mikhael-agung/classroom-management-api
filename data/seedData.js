...
async function generateMataKuliah(dosenList) {
    ...
    for (const [jurusan, matkulNames] of Object.entries(MATA_KULIAH_PER_JURUSAN)) {
        ...
        for (let i = 0; i < 25; i++) {
            ...
            const semester = Math.floor(i / 3) + 1; // Distribusi semester 1-8
            const semesterTipe = semester % 2 === 1 ? 'Ganjil' : 'Genap';
            const dosen = dosenJurusan[i % dosenJurusan.length];
            ...
            matkulList.push({
                ...
                semester: semester,
                deskripsi: `Mata kuliah ${nama} untuk jurusan ${jurusan}, semester ${semester} (${semesterTipe})`
            });
...
async function generateMahasiswaKelas(mahasiswaList, jadwalList, mataKuliahList) {
    ...
    for (const mahasiswa of mahasiswaList) {
        ...
        // Pilih jadwal secara random
        const selectedJadwal = faker.helpers.arrayElements(jadwalJurusan,
            Math.min(jumlahJadwal, jadwalJurusan.length));
